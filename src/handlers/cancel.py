...
@router.message(lambda c: c.data == "cancel")
async def cancel_button(message: types.Message) -> types.Message:
    return await cancel_command(message=message, state=None)
